...
def verify_doc_id_format(doc_id: str, expected_collection: str) -> bool:
    ...
    - All collections: {collection}_{uuid}_{timestamp}
    - memory_bank has underscore in name, so: memory_bank_{uuid}_{timestamp}
    """
    ...
    # First part after collection should be UUID (hex)
    try:
        int(parts[0], 16)
    except ValueError:
        return False

    # Second part should be timestamp (float)
    try:
        float(parts[1])
    except ValueError:
        return False
