...
class PromotionService:
    ...
    async def _handle_deletion(
        ...
        # v0.3.0 FIX: memory_bank and books are PERMANENT - never delete based on score
        # Scores are only used for ranking, not lifecycle management
        if collection in ("memory_bank", "books"):
            logger.info(f"[PROMOTION] BLOCKED deletion for {collection}/{doc_id} (score={score:.2f}) - permanent collection")
            return
