...
# Delayed import to avoid torch.testing error - only import if HF fallback is needed
...
class EmbeddingService(EmbeddingServiceInterface):
    def __init__(self, model_name: Optional[str] = None):
        ...
        self.hf_model = None
        self.client = None  # Lazy initialization - don't connect until first use
    ...
    async def _hf_embed(self, text: str) -> List[float]:
        ...
        try:
            if self.hf_model is None:
                # Lazy import to avoid torch.testing error on startup
                ...
                if SentenceTransformer is None:
                    try:
                        ...
                        logger.error(f"Cannot import sentence_transformers, HF fallback disabled: {e}")
                        # Return zero embeddings if can't import
                        return [0.0] * self._embedding_dim
