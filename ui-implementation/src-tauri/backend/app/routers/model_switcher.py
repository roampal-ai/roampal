...
async def get_current_model(request: Request):
    ...
    return {
        "current_model": current_model if llm_client_available else None,
        "can_chat": can_chat,
...
async def switch_model(request: Request, model_request: ModelSwitchRequest):
    ...
    try:
        ...
        # First verify the model exists (try common paths for Windows)
        try:
            # Try to find ollama command - check common Windows paths
            import shutil
            ...
            if not ollama_cmd:
                logger.warning("Ollama command not found in PATH or common locations - skipping verification")
                ollama_cmd = "ollama"  # Try anyway
        ...
        if hasattr(request.app.state, 'llm_client'):
            ...
            # Verify model health with test inference (30s timeout for slower systems)
            try:
                logger.info(f"Performing health check on model: {model_name}")

                # CRITICAL FIX: Recycle httpx client before health check to avoid stale connections
                # This prevents 500 errors from cached/broken connections
                if hasattr(request.app.state.llm_client, '_recycle_client'):
                    logger.info("Recycling HTTP client before health check to ensure fresh connection")
                    await request.app.state.llm_client._recycle_client()

                # Give Ollama a moment to fully load the new model before testing
                # Some systems need time between model switch and first inference
                await asyncio.sleep(2.0)
                logger.info("Starting health check after initialization delay")

                # LIGHTWEIGHT health check: Use minimal parameters to avoid memory issues
                # Don't use generate_response() which applies heavy context/generation params
                logger.info("Performing lightweight health check with minimal parameters")
                ...
                logger.error(f"Model health check timed out for {model_name} after 30 seconds")
                # Rollback to previous model
                request.app.state.llm_client.model_name = previous_model
                raise HTTPException(
                    status_code=503,
                    detail=f"Model {model_name} health check timed out after 30s. Rolled back to {previous_model}"
                )
