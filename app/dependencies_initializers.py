...
async def initialize_og_prompt_engine(config: OGPromptSettings) -> PromptEngineInterface:
    logger.info(f"Initializing PromptEngine for LoopSmith, default prompt file: {config.default_system_prompt_name}.txt")
    engine = PromptEngine()

    # Use LoopSmith template directory
    loopsmith_template_dir = settings.paths.get_soul_layers_dir()
    prompt_dirs = [str(loopsmith_template_dir), str(settings.paths.prompt_template_dir)]
